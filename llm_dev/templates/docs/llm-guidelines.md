...
| **RAG (Retrieval-Augmented Generation)** | 최신 문서/사내 데이터 기반 답변, 검증 가능한 출처 필요 시 | 최신성 유지 가능, 환각 감소, 데이터 갱신 용이 | 청킹(Chunking), 임베딩 및 검색 최적화 필요 |
| **Fine-Tuning** | 특정 스타일/태그/출력 형식 고정, 소형 모델(SLM) 최적화 | 추론 속도 개선, 프롬프트 토큰 절약 | 데이터셋 구축 비용 높음, 최신 지식 업데이트 어려움 |
| **LLM Agent & Tool-Use** | 복잡한 다단계 의사결정, 외부 API 연동 및 업무 자동화 | 자율적 문제 해결 및 실시간 외부 시스템 연동 | 환각 시 잘못된 액션 위험, 루프/비용 제어 필수 |
...
### 3.6. AI 코딩 규칙 핵심 요약 표 (Rules Summary)

| 영역 | 필수 준수 사항 | 위반 시 조치 / 영향 |
| :--- | :--- | :--- |
| **보안 (Security)** | API Key, JWT 시크릿 하드코딩 금지 (환경변수 주입) | 보안 취약점 경고 및 CI 빌드 차단 |
| **격리 (Sandbox)** | Agent 코드 실행 시 Docker Sandbox (`network_mode: none`) | 호스트 시스템 침해 및 무단 실행 위험 방지 |
| **로깅 (Logging)** | TraceID, 토큰 사용량, 지연시간 포함 JSON 구조화 로깅 | 트랜잭션 추적 불가 및 장애 분석 불가 |
| **인가 (Auth/RBAC)** | Document-level RBAC 메타데이터 필터링 및 PII 마스킹 | 타 사용자 권한 문서 유출 및 컴플라이언스 위반 |
| **품질 (Standard)** | 표준 API Response Schema 및 Pydantic/DTO 엄격 검증 | 클라이언트 연동 오류 및 비정형 데이터 오염 |
