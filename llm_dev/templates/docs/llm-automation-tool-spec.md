...
| 핵심 기능 | 세부 역할 | 주요 산출물 및 특징 |
| :--- | :--- | :--- |
| 🚀 **1. Init (스캐폴딩)** | 표준 폴더 및 인프라 문서 복사 | Docker/ELF 로깅 템플릿, AI 룰셋(`.cursorrules`), FastAPI/Spring 보일러플레이트 |
| 🔍 **2. Doctor (룰 검증)** | 4대 인프라 및 거버넌스 정적 진단 | API 키 하드코딩, PII 마스커, JSON 구조화 로깅, API 응답 포맷 준수 여부 실시간 점검 |
| 📊 **3. Tracker (진척도/대시보드)** | Day 0 ~ Day 5 라이프사이클 추적 | 마크다운 체크리스트 기반 완료율 집계 및 로컬 Glassmorphism 웹 대시보드(`view`) 서빙 |
