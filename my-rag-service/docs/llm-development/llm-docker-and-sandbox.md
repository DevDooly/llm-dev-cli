...
| 통제 영역 | 보안 통제 원칙 | 세부 설정 및 작동 방식 |
| :---: | :--- | :--- |
| **01** | 🚫 **Network Isolation** | `network_mode: none` 적용으로 외부 인터넷 및 사내망 접근 완전 차단 |
| **02** | ⏳ **Ephemeral Container** | 실행 완료 후 컨테이너 자동 즉시 파기 (`--rm` 플래그) |
| **03** | 🔒 **Read-Only Root FS** | 루트 파일시스템 쓰기 금지 (`read_only: true`), `/tmp`만 메모리(`tmpfs`)로 허용 |
| **04** | 🛑 **Resource Quota** | CPU 최대 1코어(`nano_cpus`), RAM 256MB 등 자원 고갈 공격 방지 |
| **05** | ⏱️ **Hard Timeout** | 5~10초 초과 시 SIGKILL 강제 종료 (무한 루프 방지) |
